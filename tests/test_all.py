...
from hema2.models.registry import known_profiles
from eval.harness import HarnessManifest, MemoryCase, make_request
...
def test_fact_store():
    ...
    assert len(valid) == 1 and valid[0].value == "Busan"
    at_10 = s.query_at(10, "user")
    assert len(at_10) == 1 and at_10[0].value == "Seoul"
    assert len(s) == 2
    s.add(Fact("user", "city", "Busan", t_start=60, source_turns={"t60"}))
    assert len(s) == 2
    n = s.invalidate_by_source({"t50", "t60"}, at_turn=70)
    assert n == 1 and len(s.query_valid("user")) == 0
...
def test_answer_classification():
    stale = {"Seoul", "Daejeon"}
    assert classify_current_answer("Your current city is Busan.", "Busan", stale) == "correct_latest"
    assert classify_current_answer("You moved from Seoul to Busan.", "Busan", stale) == "mixed_latest_and_stale"
    assert classify_current_answer("Your city is Seoul.", "Busan", stale) == "stale_only"
    assert classify_current_answer("UNKNOWN", "Busan", stale) == "abstained"
    metrics = aggregate_classifications(["correct_latest", "stale_only"])
    assert metrics["update_accuracy"] == 0.5 and metrics["strict_stale_rate"] == 0.5


def test_harness_manifest_and_request():
    manifest = HarnessManifest(seed=7, adaptation="self-guide")
    case = MemoryCase("tri-fact", "city", "Busan", {"Seoul"},
                      "VALID user.city = Busan", "What is my current city?", 100)
    request = make_request(case, manifest, guide="Prefer VALID facts.")
    assert request.seed == 7
    assert request.metadata["harness_version"] == manifest.version
    assert "Prefer VALID facts" in request.messages[1]["content"]
    assert len(manifest.system_prompt_sha256) == 64


def test_model_profiles():
    assert {"gpt-5.6", "fable-5", "kimi-k3", "ollama"}.issubset(set(known_profiles()))
